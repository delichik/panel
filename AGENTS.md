...
只把必须遵守的规则放在这里；项目行为合同与展开说明见 [项目设计与验收规范](docs/agents/acceptance/README.md)。

## 必读规则

- 开始任务时先读本文件，再按[验收规范索引](docs/agents/acceptance/README.md)只加载任务相关文档；用稳定验收编号确认改动范围。
...
- 修改功能时，必须同步更新相关验收合同；新增功能模块时，必须新增验收项并更新验收索引与覆盖基线。
...
发生以下任一情况时，必须更新对应验收文档、索引和覆盖基线：

- 新增、删除或重命名功能、页面、API、后台任务、配置项、数据库表或重要字段。
- 修改跨模块调用关系、持久化结构、任务流程、调度行为、远程命令行为或权限要求。
- 调整用户可见行为，即使代码入口没有变化，也要在相关验收合同中记录。
- 发现验收合同与代码不一致时，必须先确认产品意图，并在同一改动中修正错误实现或合法更新合同；不得默认以当前代码覆盖设计。
