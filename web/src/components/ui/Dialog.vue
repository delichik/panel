...
<template>
  <Teleport to="body">
    <div v-if="open" class="motion-overlay fixed inset-0 z-50 grid place-items-center bg-overlay p-4" role="presentation" @click.self="close">
      <section ref="dialog" class="motion-popover grid max-h-[min(720px,calc(100dvh-32px))] w-full max-w-lg grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden rounded-2xl border border-border bg-popover text-popover-foreground shadow-2xl" role="dialog" aria-modal="true" :aria-labelledby="titleId" :aria-describedby="description ? descriptionId : undefined" tabindex="-1" @keydown="onKeydown">
