...
export const mockFetch: typeof fetch = async (input, init = {}) => {
  ...
  if (match && method === 'GET') return envelope(state.fail2ban[match[1]] ?? { serverId: match[1], installed: false, active: false, jails: [], raw: '', configYaml: 'jails:\n  - name: sshd\n    enabled: true\n    filter: sshd\n    port: ssh\n    logpath: /var/log/auth.log\n    backend: systemd\n    maxretry: 5\n    findtime: 10m\n    bantime: 1h\n    ignoreip:\n      - 127.0.0.1/8\n', config: { jails: [{ name: 'sshd', enabled: true, filter: 'sshd', port: 'ssh', logpath: '/var/log/auth.log', backend: 'systemd', maxretry: 5, findtime: '10m', bantime: '1h', ignoreip: ['127.0.0.1/8'], options: {} }] }, updatedAt: undefined });
  if (match && method === 'PUT') { const body = await jsonBody(request); const current = state.fail2ban[match[1]] ?? { serverId: match[1], installed: true, active: true, jails: ['sshd'], raw: '', configYaml: '', config: { jails: [] } }; current.configYaml = String(body.configYaml || ''); current.installed = true; current.active = true; current.updatedAt = new Date().toISOString(); state.fail2ban[match[1]] = current; return envelope({ taskId: taskCreated('server_fail2ban_apply', match[1]).id }, 202); }
  match = path.match(/^\/servers\/([^/]+)\/fail2ban\/install$/);
  if (match && method === 'POST') { const current = state.fail2ban[match[1]]; if (current) { current.installed = true; current.active = true; } return envelope({ taskId: taskCreated('server_fail2ban_apply', match[1]).id }, 202); }
