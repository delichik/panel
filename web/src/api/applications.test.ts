...
describe('applicationsApi', () => {
  it('calls Application CRUD and validation endpoints', async () => {
    ...
    expect(fetcher).toHaveBeenNthCalledWith(9, '/api/v1/application-save-sessions/session%2F1/files/delete', expect.objectContaining({ method: 'POST', body: JSON.stringify({ path: 'config/old.conf' }) }));
    expect(fetcher).toHaveBeenNthCalledWith(10, '/api/v1/application-save-sessions/session%2F1/commit', expect.objectContaining({ method: 'POST' }));
    expect(fetcher).toHaveBeenNthCalledWith(11, '/api/v1/applications/app%201/package', expect.objectContaining({ method: 'GET' }));
    expect(fetcher).toHaveBeenNthCalledWith(12, '/api/v1/applications/app%201/validate', expect.objectContaining({ method: 'POST' }));
    expect(fetcher).toHaveBeenNthCalledWith(13, '/api/v1/applications/app%201/plan', expect.objectContaining({ method: 'POST' }));
    expect(fetcher).toHaveBeenNthCalledWith(14, '/api/v1/applications/app%201/image/check', expect.objectContaining({ method: 'POST' }));
  });
