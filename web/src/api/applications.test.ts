...
describe('applicationsApi', () => {
  it('calls Application CRUD and validation endpoints', async () => {
    ...
    await api.getFile('app 1', 'file/1');
    ...
    expect(fetcher).toHaveBeenNthCalledWith(5, '/api/v1/applications/app%201/files/file%2F1', expect.objectContaining({ method: 'GET' }));
    expect(fetcher).toHaveBeenNthCalledWith(6, '/api/v1/applications/app%201/files', expect.objectContaining({ method: 'POST', body: JSON.stringify({ path: 'config/app.conf', kind: 'template', contentBase64: 'aGVsbG8=' }) }));
    expect(fetcher).toHaveBeenNthCalledWith(7, '/api/v1/applications/app%201/files/file%2F1', expect.objectContaining({ method: 'DELETE' }));
    expect(fetcher).toHaveBeenNthCalledWith(8, '/api/v1/application-save-sessions', expect.objectContaining({ method: 'POST', body: JSON.stringify({ applicationId: 'app 1', save: input }) }));
    expect(fetcher).toHaveBeenNthCalledWith(9, '/api/v1/application-save-sessions/session%2F1/files', expect.objectContaining({ method: 'POST' }));
    expect(fetcher).toHaveBeenNthCalledWith(10, '/api/v1/application-save-sessions/session%2F1/files/archive', expect.objectContaining({ method: 'POST', body: expect.any(FormData) }));
    expect(fetcher).toHaveBeenNthCalledWith(11, '/api/v1/application-save-sessions/session%2F1/files/delete', expect.objectContaining({ method: 'POST', body: JSON.stringify({ path: 'config/old.conf' }) }));
    expect(fetcher).toHaveBeenNthCalledWith(12, '/api/v1/application-save-sessions/session%2F1/commit', expect.objectContaining({ method: 'POST' }));
    expect(fetcher).toHaveBeenNthCalledWith(13, '/api/v1/applications/app%201/package', expect.objectContaining({ method: 'GET' }));
    expect(fetcher).toHaveBeenNthCalledWith(14, '/api/v1/applications/app%201/validate', expect.objectContaining({ method: 'POST' }));
    expect(fetcher).toHaveBeenNthCalledWith(15, '/api/v1/applications/app%201/plan', expect.objectContaining({ method: 'POST' }));
    expect(fetcher).toHaveBeenNthCalledWith(16, '/api/v1/applications/app%201/image/check', expect.objectContaining({ method: 'POST' }));
  });
