...
} from '../_shared/addressOptions';

const router = useRouter();
const route = useRoute();
const { t, translateNomadNodeKind, translateNomadNodeRole, translateNomadNodeStatus } = useI18n();
...
<template>
  <div class="page-shell">
    ...
    <v-card variant="outlined" :loading="loading" class="nodes-card">
...
.nodes-card { display: flex; flex: 1 1 auto; flex-direction: column; min-height: 0; overflow: hidden; }
.app-card-header { display: flex; justify-content: flex-end; gap: 8px; flex-wrap: wrap; }
.mono { font-size: 0.8rem; }
.node-table-wrap { min-height: 0; overflow: auto; }
