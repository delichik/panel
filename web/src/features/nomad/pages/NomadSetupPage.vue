...
async function load() {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : 'Unable to load Nomad setup state';
  } finally {
    loading.value = false;
  }
}
...
async function bootstrapSelectedServer() {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : 'Unable to bootstrap Nomad server';
  } finally {
...
<template>
  <div>
    <div class="page-heading mb-5">
      <div>
        <div class="eyebrow">Nomad setup</div>
        <h1 class="text-h4 font-weight-bold">Bootstrap first Nomad server</h1>
      </div>
    ...
    <v-card variant="outlined" class="setup-panel">
      ...
      <div class="setup-copy">
        <div class="text-h6 font-weight-bold">Select one SSH Server</div>
    ...
    <v-card v-if="activeTaskId" class="mt-4 pa-4" variant="outlined">
      <v-card-title class="px-0 pt-0 text-subtitle-1 font-weight-bold">Bootstrap Task</v-card-title>
