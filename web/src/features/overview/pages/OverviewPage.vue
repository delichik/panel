...
const cardPresets: CardPreset[] = [
  { kind: 'cpu', title: 'CPU', icon: 'mdi-cpu-64-bit', color: 'primary', category: 'metric', width: 3, height: 2 },
  { kind: 'memory', title: 'Memory', icon: 'mdi-memory', color: 'success', category: 'metric', width: 3, height: 2 },
  { kind: 'disk', title: 'Disk', icon: 'mdi-harddisk', color: 'warning', category: 'metric', width: 3, height: 2 },
  { kind: 'network', title: 'Network', icon: 'mdi-lan', color: 'info', category: 'metric', width: 3, height: 2 },
  { kind: 'packageUpdates', title: 'Package updates', icon: 'mdi-package-variant-plus', color: 'warning', category: 'message', width: 3, height: 2 },
  { kind: 'containerUpdates', title: 'Container updates', icon: 'mdi-docker', color: 'primary', category: 'message', width: 3, height: 2 },
  { kind: 'placeholder', title: 'Placeholder', icon: 'mdi-border-none-variant', color: 'secondary', category: 'message', width: 3, height: 2 },
];
...
function loadCards(): OverviewCardConfig[] {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as OverviewCardConfig[];
...
function normalizeCard(card: OverviewCardConfig): OverviewCardConfig | null {
  const preset = presetFor(card.kind);
  if (!preset) return null;
  const range = rangeItems.value.some((item) => item.value === card.range) ? card.range : '1h';
...
function saveCard() {
  const preset = presetFor(form.kind);
