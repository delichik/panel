...
var (
	...
	codeTranslations = map[string]map[string]string{
		...
		"key_asset_operation_in_progress": "该密钥资产已有操作正在进行",
			...
			"invalid_runtime_event_retention":                        "运行时事件保留时间至少为 1 天",
			"invalid_runtime_event_detail_retention":                 "运行时事件详情保留时间至少为 1 天",
			"invalid_runtime_event_retention_order":                  "运行时事件保留时间必须大于或等于详情保留时间",
			"invalid_metrics_interval":                               "指标采集间隔至少为 10 秒",
			"invalid_cleanup_schedule":                               "清理计划必须为每小时、每天或每周",
			"invalid_runtime_event_cleanup_schedule":                 "运行时事件清理计划必须为每小时、每天或每周",
			...
			"server_invalid":                                         "服务器名称和 credentialId 不能为空",
			"server_host_derived":                                    "连接地址由 ipv4/ipv6 派生，不能直接填写 host",
			"server_address_required":                                "ipv4 和 ipv6 至少需要填写一个",
			"server_ipv4_invalid":                                    "ipv4 必须是有效的 IPv4 地址",
			"server_ipv6_invalid":                                    "ipv6 必须是有效的 IPv6 地址",
			...
			"dns_record_cname_exists":                                "该主机名已存在 CNAME 记录",
			"dns_record_conflict":                                    "该主机名已存在其他类型的 DNS 记录，无法与 CNAME 共存",
			...
			"application_reverse_proxy_path_invalid":                 "反向代理路径无效",
			...
			"application_cross_module_insights_unavailable":          "跨模块的 DNS、证书和网关信息在当前 API 版本中不可用",
			"application_edit_file_hash_mismatch":                    "应用编辑会话文件校验值不匹配",
			"application_edit_file_missing":                          "应用编辑会话文件缺失",
			"application_edit_file_size_mismatch":                    "应用编辑会话文件大小不匹配",
			"application_edit_operation_conflict":                    "该编辑操作已存在",
			"application_edit_operation_invalid":                     "必须提供应用名称和客户端操作 ID",
			"application_edit_session_not_discardable":               "当前应用编辑会话不可丢弃",
			"application_edit_session_state_invalid":                 "应用编辑会话状态无效，无法操作",
			"application_file_archive_limits_exceeded":               "应用文件压缩包超出文件数、深度、解压大小或压缩率限制",
			"application_file_too_large":                             "文件超过 64 MiB 限制",
			"application_task_target_required":                       "必须选择应用任务目标",
			...
			"facility_static_asset_name_duplicate":                   "静态资产名称不能重复",
			...
			"facility_static_asset_referenced_after_delete":          "仍有路由引用了已删除的静态资产",
			"facility_cross_module_insights_stale":                   "DNS、证书、防火墙和网关端口的相关信息暂时不可用",
			"facility_route_summary_stale":                           "路由摘要暂时无法刷新",
			"facility_apply_request_failed":                          "配置已提交，但无法请求应用",
			"facility_gateway_removal_invalidates_origin":            "移除网关节点会移除已配置的域名源站",
			"facility_gateway_removal_invalidates_anyaccess_primary": "移除网关节点会移除 AnyAccess 主源站",
			"facility_gateway_removal_invalidates_panel_entry":       "Panel 入口服务器必须保留在网关节点集合中",
			"facility_commit_recovered":                              "重启后已恢复已提交的设施配置",
			"panel_host_provider_unavailable":                        "Panel 宿主节点注册不可用",
			"client_operation_id_required":                           "必须提供客户端操作 ID",
			"edit_session_revision_conflict":                         "编辑会话已在其他客户端被修改",
			"facility_asset_content_missing":                         "设施资产内容缺失或不可读",
			"facility_asset_hash_mismatch":                           "编辑期间设施资产内容已变更",
			"facility_commit_lease_lost":                             "编辑提交租约已失效，请重试提交",
			"facility_edit_asset_invalid":                            "资产名称、类型和内容不能为空",
			"facility_edit_asset_kind_immutable":                     "资产类型不可更改，请删除后重新创建",
			"facility_edit_asset_mode_immutable":                     "资产内容模式不可更改，请删除后重新创建",
			"facility_edit_asset_mode_invalid":                       "压缩包资产必须使用二进制模式",
			"facility_edit_asset_operation_invalid":                  "必须提供资产名称和客户端操作 ID",
			"facility_edit_operation_conflict":                       "该编辑操作已存在",
			"facility_edit_session_not_discardable":                  "当前编辑会话不可丢弃",
			"facility_edit_session_state_invalid":                    "编辑会话状态无效，无法读取",
			"facility_edit_text_invalid":                             "文本资产必须是有效的 UTF-8 且不超过 1 MiB",
			"facility_reverse_proxy_invalid":                         "设施编辑会话无效，无法提交",
			"facility_dns_sync_failed":                               "配置已提交，但请求 DNS 记录同步失败",
			"facility_source_asset_missing":                          "设施源资产内容缺失或不可读",
			"facility_static_asset_archive_limits_exceeded":          "静态资产压缩包超出文件数、深度、解压大小或压缩率限制",
			"facility_static_asset_content_invalid":                  "静态资产内容无效",
			"idempotency_key_required":                               "必须提供 Idempotency-Key 请求头",
			"idempotency_key_reused":                                 "幂等键已被使用或与请求不匹配",
			"preview_stale":                                          "编辑预览缺失或已过期，请重新预览",
			"resource_version_conflict":                              "配置在编辑期间已被修改，请刷新后重试",
			"resource_version_invalid":                               "基础资源版本无效",
	...
	exactTranslations = map[string]map[string]string{
		LocaleSimplifiedChinese: {
			...
			"A CNAME record with that host already exists":                                  "该主机名已存在 CNAME 记录",
			"A record with a different type already exists for this host":                   "该主机名已存在其他类型的 DNS 记录，无法与 CNAME 共存",
			...
			"Server name and credentialId are required":                              "服务器名称和 credentialId 不能为空",
			"Server host is derived from ipv4/ipv6 and cannot be set directly":       "连接地址由 ipv4/ipv6 派生，不能直接填写 host",
			"At least one of ipv4 or ipv6 is required":                               "ipv4 和 ipv6 至少需要填写一个",
			"ipv4 must be a valid IPv4 address":                                      "ipv4 必须是有效的 IPv4 地址",
			"ipv6 must be a valid IPv6 address":                                      "ipv6 必须是有效的 IPv6 地址",
			"Configuration was committed, but requesting the DNS record sync failed": "配置已提交，但请求 DNS 记录同步失败",
			"Configuration was recovered, but requesting the DNS record sync failed": "配置已恢复，但请求 DNS 记录同步失败",
			...
			"environment variable names must be non-empty and must not contain =":              "环境变量名不能为空且不能包含 =",
			...
			"static port is duplicated":                                                        "静态端口重复",
			...
			"volume source must be a valid Docker volume name":                                 "卷来源必须是有效的 Docker 卷名称",
			...
			"mount target cannot be the filesystem root":                                       "挂载目标不能是文件系统根目录",
			...
			"static asset not found":                                                           "静态资产不存在",
			"facility_edit_session not found":                                                  "设施编辑会话不存在",
			"facility_edit_session_asset not found":                                            "编辑会话资产不存在",
			"facility_edit_session_asset_content not found":                                    "编辑会话资产内容不存在",
			"facility_static_asset_content not found":                                          "静态资产内容不存在",
			"facility_app_lifecycle_operation not found":                                       "设施应用操作不存在",
			"application not found":                                                            "应用不存在",
			"application edit session not found":                                               "应用编辑会话不存在",
			"application edit session file not found":                                          "应用编辑会话文件不存在",
			"application edit session is invalid":                                              "应用编辑会话无效，无法提交",
			"application file not found":                                                       "应用文件不存在",
			"application instance not found":                                                   "应用实例不存在",
