...
var (
	...
	codeTranslations = map[string]map[string]string{
		LocaleSimplifiedChinese: {
			...
			"invalid_panel_domain":                                   "Panel 域名必须是有效的主机名或 IP 地址",
			"invalid_panel_tls_certificate":                          "Panel TLS 证书无效或不匹配当前域名",
			...
			"ufw_rule_target_unknown":                                "无法确定 UFW 规则的目标端口，已拒绝删除",
			"ufw_port_invalid":                                       "UFW 端口必须在 1 到 65535 之间",
			"ufw_protocol_invalid":                                   "UFW 协议必须为 tcp、udp 或 any",
			"ufw_ssh_port_protected":                                 "不能修改节点的 SSH 防火墙端口",
			"ufw_agent_port_protected":                               "不能修改节点的 Agent 防火墙端口",
			"ufw_application_rule_managed":                           "应用受管的 UFW 规则不能手工删除",
			"firewall_ssh_preflight_failed":                          "防火墙变更要求 SSH 连接正常",
			"firewall_agent_preflight_failed":                        "防火墙变更要求 Agent 连接正常",
			"firewall_channel_preflight_failed":                      "防火墙变更前的管理通道检查失败",
			"firewall_channel_postflight_failed":                     "防火墙变更后的管理通道检查失败",
			"application_ssh_port_reserved":                          "应用不能占用节点的 SSH 端口",
			"application_agent_port_reserved":                        "应用不能占用节点的 Agent 端口",
			...
			"application_persistent_delete_confirmation_required":    "该应用可能存在持久化数据，删除前必须确认",
			"application_persistent_server_required":                 "请选择要下载持久化数据的节点",
			"application_persistent_server_invalid":                  "所选节点不是该应用已记录的持久化数据节点",
			...
			"storage_share_agent_required":                           "存储共享需要支持存储能力的 Agent，请升级该服务器上的 Agent",
			"storage_share_config_required":                          "存储共享至少需要一台配置了根目录的存储服务器",
			"storage_share_root_invalid":                             "存储根目录必须是绝对 Linux 路径且不含空格",
			"storage_share_version_conflict":                         "存储共享配置已被其他请求修改，请刷新后重试",
			"storage_share_root_immutable":                           "启用后不能更改存储根目录，请先卸载再重新启用",
			"storage_share_not_configured":                           "存储共享设施尚未配置",
			"storage_share_task_unavailable":                         "任务服务不可用",
			"storage_share_unavailable":                              "存储共享设施未配置",
			"storage_share_partition_invalid":                        "存储共享分区标识无效",
			"storage_share_server_invalid":                           "存储共享挂载源引用了不属于该设施的服务器",
			"storage_share_source_invalid":                           "存储共享挂载源必须引用存储共享设施",
			"storage_share_partition_path_invalid":                   "存储共享分区路径无效",
			"storage_share_partition":                                "存储共享分区未找到",
			...
			"execution_resolution_unsupported":                       "该 Agent 不支持人工结果核对，请先升级 Agent。",
			"execution_resolution_unavailable":                       "当前无法连接 Agent 提交核对结果，请稍后重试。",
			"execution_resolution_unconfirmed":                       "人工核对结果尚未完整保存，请保持当前执行保护并稍后重试。",
			"execution_result_unverified":                            "执行结果尚未确认，请先核对远端状态，不能直接重试。",
			"execution_verification_requires_user":                   "需要已登录的用户核对此次执行结果。",
			"execution_verification_invalid":                         "请选择核对结果并填写核对依据。",
			"execution_not_uncertain":                                "只有等待核对的执行才能提交核对结果。",
			"activity_boolean_invalid":                               "筛选开关必须为 true 或 false。",
			"activity_context_invalid":                               "上下文范围无效，最多读取 500 条。",
			"activity_limit_invalid":                                 "每批记录数量必须为 1 到 500。",
			"activity_query_invalid":                                 "日志查询包含未知或重复参数。",
			"activity_sequence_invalid":                              "日志序号必须为非负整数。",
			"activity_time_invalid":                                  "时间必须使用 RFC3339 格式。",
			"activity_time_range_invalid":                            "开始时间不能晚于结束时间。",
			"activity_batch_too_large":                               "一次事务最多追加 500 条事件。",
			"activity_event_type_required":                           "必须提供事件类型。",
			"activity_capacity_exhausted":                            "审计存储可用空间不足 1 GiB，已暂停接收新变更；请扩容后重试。",
			"activity_write_unavailable":                             "无法保存审计记录，本次操作尚未开始。",
			"activity_result_unrecorded":                             "操作可能已完成，但结果记录失败；请先核对资源状态，不要直接重试。",
			"activity_event_id_conflict":                             "事件标识已存在且内容不同，禁止覆盖日志。",
			"activity_source_sequence_conflict":                      "事件源序号冲突，记录尚未被接受。",
			"activity_index_pending":                                 "记录已经保存，查询索引正在更新。",
			"activity_cursor_invalid":                                "分页位置与当前筛选不匹配，请刷新列表。",
			"activity_evidence_pending":                              "证据仍在收集，尚未完成封存。",
	...
	exactTranslations = map[string]map[string]string{
		LocaleSimplifiedChinese: {
			...
			"Unable to resolve UFW rule target; refusing deletion":                          "无法确定 UFW 规则的目标端口，已拒绝删除",
			"UFW port must be between 1 and 65535":                                          "UFW 端口必须在 1 到 65535 之间",
			"UFW protocol must be tcp, udp, or any":                                         "UFW 协议必须为 tcp、udp 或 any",
			"The SSH firewall port cannot be modified":                                      "不能修改节点的 SSH 防火墙端口",
			"The Panel Agent firewall port cannot be modified":                              "不能修改节点的 Agent 防火墙端口",
			"Application-managed UFW rules cannot be deleted manually":                      "应用受管的 UFW 规则不能手工删除",
			"UFW is not installed; install it before opening application ports":             "尚未安装 UFW，请先安装再开放应用端口",
			"SSH connectivity verification is unavailable":                                  "SSH 连接检查不可用",
			"Panel Agent connectivity is required for firewall changes":                     "防火墙变更要求 Agent 连接正常",
			"Panel Agent connectivity must be healthy before and after firewall changes":    "防火墙变更前后 Agent 连接必须正常",
			...
			"port protocol must be tcp or udp":                                                 "端口协议必须为 tcp 或 udp",
			"firewall access requires a static port":                                           "防火墙开放要求配置固定宿主机端口",
	...
	codePrefixTranslations = map[string]map[string]map[string]string{
		LocaleSimplifiedChinese: {
			...
			"agent_request_invalid": {
				"Agent request invalid: ": "Agent 请求无效：",
			},
			"agent_request_forbidden": {
				"Agent request forbidden: ": "Agent 请求被拒绝：",
			},
			"ssh_host_key_mismatch": {
				"ssh host key mismatch: ": "SSH 主机密钥不匹配：",
			},
			"ssh_host_key_verification_failed": {
				"ssh host key verification failed: ": "SSH 主机密钥校验失败：",
			},
			"storage_share_server_in_use": {
				"storage server is still referenced by applications: ": "存储服务器仍被应用引用：",
			},
			"storage_share_in_use": {
				"Storage share is still used by applications: ":          "存储共享仍被应用使用：",
				"storage share is still mounted by running applications": "存储共享仍被运行中的应用挂载，请先停止或移除挂载",
			},
			"storage_share_partition_in_use": {
				"storage share partition is still used by application ":                                 "存储共享分区仍被应用使用：",
				"partition is still mounted by the running application; stop or remove the mount first": "分区仍被运行中的应用挂载，请先停止或移除挂载",
			},
			"query_parameter_unknown": {
				"Unknown query parameter: ": "未知查询参数：",
			},
		},
	}
