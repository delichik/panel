...
func TestIssuedCertificatesExposeApplicationInternalFiles(t *testing.T) {
	...
	if _, err := svc.db.ExecContext(ctx, `INSERT INTO applications(id,name,enabled,spec_yaml,deployment_mode,deployment_server_ids_json,generation,spec_hash,job_id,namespace,created_at,updated_at) VALUES('app_cert_file','cert file app',0,?,'all','[]',1,'hash','job','default','now','now')`,
