...
func (h *Handler) RegisterRoutes(mux *http.ServeMux, auth httpx.Middleware) {
	...
	mux.Handle("GET /api/v1/applications/{id}/files/{name}", auth(http.HandlerFunc(h.GetFile)))
	mux.Handle("GET /api/v1/applications/{id}/files/{name}/content", auth(http.HandlerFunc(h.DownloadFile)))
	mux.Handle("DELETE /api/v1/applications/{id}/files/{name}", auth(http.HandlerFunc(h.DeleteFile)))
