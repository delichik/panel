...
func (h *Handler) Get(w http.ResponseWriter, r *http.Request) {
	assetID, ok := keyAssetResourceID(r.URL.Path)
	if !ok {
...
func (h *Handler) Reissue(w http.ResponseWriter, r *http.Request) {
	assetID, ok := keyAssetActionID(r.URL.Path, "reissue")
	if !ok {
		httpx.Error(w, panelerr.NotFound("route"))
		return
	}
...
func (h *Handler) Regenerate(w http.ResponseWriter, r *http.Request) {
	assetID, ok := keyAssetActionID(r.URL.Path, "regenerate")
	if !ok {
		httpx.Error(w, panelerr.NotFound("route"))
		return
	}
...
func (h *Handler) DownloadFile(w http.ResponseWriter, r *http.Request) {
	assetID, kind, ok := keyAssetFileResource(r.URL.Path)
	if !ok {
		httpx.Error(w, panelerr.NotFound("route"))
		return
	}
...
func (h *Handler) DownloadExport(w http.ResponseWriter, r *http.Request) {
	taskID, ok := keyAssetExportTaskID(r.URL.Path)
	if !ok {
		httpx.Error(w, panelerr.NotFound("route"))
		return
	}
...
func (h *Handler) ExecuteImport(w http.ResponseWriter, r *http.Request) {
	planID, ok := keyAssetImportPlanID(r.URL.Path)
	if !ok {
		httpx.Error(w, panelerr.NotFound("route"))
		return
	}
...
func keyAssetResourceID(path string) (string, bool) {
	parts := pathParts(path)
	if len(parts) == 4 && parts[0] == "api" && parts[1] == "v1" && parts[2] == "key-assets" && parts[3] != "" {
		return parts[3], true
	}
	return "", false
}

func keyAssetActionID(path, action string) (string, bool) {
	parts := pathParts(path)
	if len(parts) == 5 && parts[0] == "api" && parts[1] == "v1" && parts[2] == "key-assets" && parts[3] != "" && parts[4] == action {
		return parts[3], true
	}
	return "", false
}

func keyAssetFileResource(path string) (string, string, bool) {
	parts := pathParts(path)
	if len(parts) == 6 && parts[0] == "api" && parts[1] == "v1" && parts[2] == "key-assets" && parts[3] != "" && parts[4] == "files" && parts[5] != "" {
		return parts[3], parts[5], true
	}
	return "", "", false
}

func keyAssetExportTaskID(path string) (string, bool) {
	parts := pathParts(path)
	if len(parts) == 6 && parts[0] == "api" && parts[1] == "v1" && parts[2] == "key-assets" && parts[3] == "exports" && parts[4] != "" && parts[5] == "download" {
		return parts[4], true
	}
	return "", false
}

func keyAssetImportPlanID(path string) (string, bool) {
	parts := pathParts(path)
	if len(parts) == 6 && parts[0] == "api" && parts[1] == "v1" && parts[2] == "key-assets" && parts[3] == "imports" && parts[4] != "" && parts[5] == "execute" {
		return parts[4], true
	}
	return "", false
}
