...
4. 应用网络固定使用受管 `panel-apps` bridge 网络，无需配置。
5. 新增 TCP 端口映射，将容器端口 `80` 映射到未占用的主机端口，例如 `8081`。可选择开启**管理防火墙规则**，由 Panel 在每个部署节点维护对应的 UFW 允许规则。
...
如果目标服务器启用了防火墙且该映射未由 Panel 管理规则，请在 **资源 → 防火墙** 放行所选宿主机端口，或者使用反向代理，避免直接公开测试端口。应用部署不会自动安装或启用 UFW；应用自动规则和人工防火墙操作都不能以节点 SSH 或 Agent 端口为目标。每次防火墙写入前后都必须实时确认 SSH 与 Agent 两条连接可用。

## 6. 配置实际应用

应用编辑器使用结构化配置表单。常用配置包括：

- 容器镜像和命令。
- CPU、内存、特权模式和 Linux capability。
- 容器环境变量。
- TCP/UDP 端口映射和可选的 Panel 受管 UFW 规则（应用网络始终为受管 `panel-apps` bridge 网络）。受管规则要求固定宿主机端口，并会在应用停用、删除或移出部署节点时清理；已有人工规则不会被删除。
