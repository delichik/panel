...
- 状态型详情页若编辑被拆到独立配置路由，`AppDetailPanel` 正文只展示摘要、路由/关联对象概览和运行记录；标题区仅保留同步、打开配置等对象级动作，不得继续渲染旧表单或保存按钮。
- 详情卡通常 `padding: 16px`。
...
- 主从页面的复杂详情仍应保持单一外层详情卡；基本信息、镜像、运行实例等内容使用无外层阴影的正文分区，不为每个分区再套一张同等级 outlined 卡片。
