...
- 设施静态/重定向/手工代理 Path 与普通应用代理 Path 复用 `RoutePathAdvancedFields.vue`，不得在两个业务页面各复制一套 Gzip、请求体、超时、缓冲、WebSocket 和 Header 表单。
- 共享组件只接收 `HTTPRouteOptionsDto` 以及当前 Path 是否支持代理/Gzip的能力参数；域名、目标端口、静态来源、重定向地址和上游 URL 留在业务对话框中。
- 不适用于当前路由类型的字段不显示，并由后端规范化清零或恢复继承值。页面不得提供 Nginx `proxy_cache` 开关；Panel 生成的代理 location 固定禁用网关响应缓存，客户端缓存 Header 不属于专用表单能力。
- Header 重复行使用名称和值两列以及行尾删除动作；请求 Header 允许空值表示移除，响应 Header 必须有值。所有字段保存稳定值，不保存翻译后的模式标题。
