...
服务器、域名、防火墙、fail2ban、软件包、应用、设施应用、证书和任务等主从页面统一使用 `AppMasterDetailWorkspace.vue`：

```html
<AppMasterDetailWorkspace>
  <template #aside>
    <!-- AppSelectorPanel 或 ServerSelector -->
  </template>
  <!-- 右侧详情或表格工作区 -->
</AppMasterDetailWorkspace>
```
...
- 新增主从页面不得在 feature 页面内复制 `grid-template-columns: clamp(300px, 26vw, 340px)`；如需改变断点或栏宽，先调整共享组件和本文档。
...
- 设置、账户、安全参数等长表单不应在超宽工作区中铺满整行；页面卡片宽度应收敛到适合阅读和编辑的范围。
- 卡片内部可使用固定分类标题区，正文表单作为内部滚动区；每个保存按钮只保存所属 section。
- 分区块使用弱边框和弱表面背景表达编辑范围，避免靠大量横线切碎表单。

## 资源型主从页

- 容器、镜像、网络、卷等资源页右侧详情应先展示当前服务器上下文头，包括名称、地址和可达状态。
- 表格、工具栏和操作区位于上下文头下方的内部内容区，内容区独立滚动。
- 不应让右侧表格直接贴在无语义大卡片顶部，用户需要先知道当前操作的是哪台服务器。
