...
- 当主从详情承担运行状态和配置概览、但编辑容易误触时，详情页保持只读并通过详情标题区动作进入隐藏独立配置路由。只读详情继续使用 `AppDetailPanel`，不得为了复用旧实现把表单控件隐藏在正文中。
- 独立配置页使用 `.page-shell` 加一张吸收剩余高度的主工作面；页面工具栏和主卡片标题固定，配置正文内部滚动。所有分区共享一份本地草稿和一个最终提交动作时，保存按钮放在主工作面标题区。
- 保存期间使用 contained persistent overlay 覆盖主工作面，展示当前阶段并阻止返回、路由离开和重复提交；窄屏仍按 `760px` 规则恢复页面自然高度。
- 多分区独立配置页可复用应用编辑器的左侧分区导航：左侧 sticky 导航，右侧所有分区共享一个内部滚动容器，点击导航平滑定位并通过滚动观察更新高亮。设施入口网关配置页采用该模式，配置页不得混入只读来源的数据。
- 设施入口网关中的 Panel 入口节点属于 setup 生成的系统关系，以 disabled 选择器展示当前宿主节点；未完成 setup 时使用紧凑 info alert 说明并禁用入口开关，不另建新的配置卡片。
