...
| 操作分级、列表查询进 URL、编辑脏状态、异步两阶段、反馈分层、加载骨架屏 | [interaction-model.md](interaction-model.md) | 页面族目录、`web/src/components/ui/` |
...
1. 写页面先选模板：Dashboard / List / MasterDetail / Editor / Settings / Workspace。阶段占位只允许在页面族接入前短期存在，交付页不得回退成通用 CollectionPage。
...
4. 页面族必须独立：overview、servers、credentials、security、resources、applications、dns、certificates、tasks、settings、auth、maintenance、debug 都有独立入口目录。可以复用模板和 primitives，但内容结构、操作闭环和 API 模块必须按业务任务设计。
