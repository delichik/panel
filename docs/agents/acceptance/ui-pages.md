...
| UI-APP-007 | 点击“容器日志” | 立即打开 Dialog 并加载该应用实例；按服务器名/容器名选择目标，单个可读实例自动加载，多个等待选择；请求携带 instanceId、tail=240，正文内部滚动并支持手动刷新/刷新实例列表 | 已退出但存在的容器仍可选；无实例、不可读、空日志和读取失败分开；失败 toast 加就地重试；切换实例/应用/关闭时取消旧请求且迟到结果不串日志 | 只请求选中实例，不通过任务或协调记录代替容器日志；不定时轮询，长日志不撑开页面 |
...
| UI-APP-009 | 删除应用 | 普通应用显示影响说明并 danger 确认；存在持久化配置/历史位置时使用 danger 强制勾选确认，明确说明将永久删除所有已记录节点的数据，并向后端传确认参数 | 取消零请求；未确认或清理失败不移除对象 | 持久化删除只有勾选后发出，成功后重载列表且已删除应用不再选中 |
| UI-APP-010 | 下载持久化数据 | 展示历史数据节点选择器；单节点自动选择，多节点要求用户选择，携带 serverId 走授权 blob 下载 | 无记录位置或未选择节点时禁用；下载失败不生成损坏文件 | 未 apply 与已移除部署节点的数据均可按节点下载，按钮有单独 loading |
...
### UI-ACT-002 Activity 首次加载

- **前置**：认证用户进入 `/activity`，URL 可能带完整查询条件，也可能缺少 `from`。
- **动作**：Activity 页面首次挂载。
- **结果**：必须立即按 URL 条件请求当前事件/操作视图的列表；缺少 `from` 时使用最近 24 小时并把该时间写回 URL，不请求全量摘要统计。
- **失败**：默认时间写回不得替代、延迟或重复首次列表请求；失败保留错误空态、toast 和重试入口。
- **不变量**：首次列表加载不依赖定时器，不启动 tail/summary 轮询。
- **验证**：activity initial-load 组件/逻辑测试覆盖无 `from` 和显式范围。

### UI-ACT-003 日志只按需刷新

- **前置**：用户浏览日志或选中操作详情，可能保留其它页面的全局自动刷新设置。
- **动作**：首次进入、手动刷新、修改查询条件、选择对象或翻页。
- **结果**：仅这些动作读取对应列表/详情/上下文；不提供定时刷新开关，不读取或探测新数据，不显示总条数、错误/警告计数、新数据提示或快照编号。索引未就绪时可使用现有列表元数据简短提示，不额外请求。
- **失败**：查询失败仍显示错误与手动重试；旧筛选、旧选择和旧上下文响应不得覆盖当前结果。没有自动错误重试或页面重新可见时的隐式轮询。
- **不变量**：列表和详情保持快照游标分页及有界 DOM；手动刷新/导出继续使用既有快照语义。页面不调用 summary/tail；不为容量横幅保留统计查询，后端容量准入和结构化写入错误不受影响。共享自动刷新仍可供其它页面使用。
- **验证**：`views/activity/index.test.ts` 覆盖首屏一次请求、空闲一分钟零追加请求、手动列表/详情刷新、游标翻页及迟到结果隔离。
...
`UI-FAC-012`：入口代理按节点显示容器观测与部署结果、失败阶段、下次重试及活动入口；错误带原因与处理提示，技术诊断/本次运行日志放入内部可滚动折叠区。按需代理检查显示样本限制、节点/域名/上游、分类解释与下一步；空样本不表示全部路由健康，旧配置/离页响应不得覆盖当前结果。
