...
- 全量备份与还原由 `internal/modules/backups` 提供。正常运行期的备份导出只写 pending export 并提示重启；真正归档在下一次启动早期由 `ExportApp` 执行。`cmd/panel/main.go` 在正常 bootstrap 前先检查 `data/tmp/restore-pending/pending.json`，再检查 `data/tmp/backup-export-pending/pending.json`；命中任一维护作业时只启动对应维护 HTTP 服务，不打开正常业务数据库、不装配业务模块、不启动后台 worker。备份导出完成后提供下载并要求重启；恢复成功后删除 pending 文件并要求重启。
