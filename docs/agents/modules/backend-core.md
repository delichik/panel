...
- `app.New` 还会初始化 `internal/agent` 的专用 mTLS 资产，路径为 `<dataRoot>/agent/tls`；该 CA 只用于 Panel 与目标机 agent 的双向认证，不与用户密钥资产混用。服务启动后会后台扫描服务器，调度器也会周期检查已配置 agent：未配置 agent 的服务器会自动创建部署任务安装 agent；已配置 agent 的服务器会检查版本、Docker host、能力和证书时间错误，并把结果写入服务器 traits；发现 `agent.status=incompatible`，或健康检查、应用运行时、容器化资源、指标采集等 Agent API 因 agent mTLS server 证书过期/尚未生效失败时，会自动创建并启动 agent 部署任务重装目标机 agent。依赖 agent 的定时工作必须只在 `agent.status=compatible` 且存在 `agent.url` 时执行；agent 不可用时跳过当前工作，不创建资源操作任务，也不回退 SSH。自动部署同一服务器连续失败达到上限后会停止自动尝试，等待用户从服务器详情手动重装。
