...
- `app.New` 还会初始化 `internal/agent` 的专用 mTLS 资产，路径为 `<dataRoot>/agent/tls`；该 CA 只用于 Panel 与目标机 agent 的双向认证，不与用户密钥资产混用。目标机 agent 默认监听 `tcp/9786`。服务启动后会后台扫描服务器，调度器也会周期检查已配置 agent：未配置 agent、`agent.status=unavailable` 异常和 `agent.status=incompatible` 不兼容的服务器会自动创建或复用部署任务安装/修复 agent；连续 2 次系统自动部署失败后写入 `agent.status=undeployable` 并停止自动部署。依赖 agent 的定时工作必须只在 `agent.status=compatible` 且存在 `agent.url` 时执行；agent 不正常时跳过当前工作，不创建资源操作任务，也不回退 SSH。手动重装会清除自动部署停止标记。
