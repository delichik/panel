...
- UFW 的普通人工写操作和应用受管规则都必须经过管理端口安全门：目标端口等于节点当前 SSH 端口或实际 Agent URL 端口时拒绝添加、删除或接管；按编号删除前必须在节点侧重新读取并解析目标规则，无法可靠解析端口时按安全原则拒绝。UFW 安装/启用流程中“先确保 SSH 与 Agent allow”是唯一内部例外，且绝不删除这两条管理规则。
- 所有 UFW 写操作（人工添加/删除、安装/启用以及应用 `RuntimeReconcile` 的规则确保/清理）在修改前后都必须实时验证 SSH 与 Agent 两条通道可建立新连接；任一通道不可用时不得开始修改，修改后的复查失败必须报告错误，不能静默承诺节点仍可管理。该门禁使用实时探测，不以数据库中的历史 reachable/compatible 标志替代。
- 应用端口的受管 UFW 规则由 Agent 在 `RuntimeReconcile` 内协调：规则使用稳定 comment 标识应用所有权，UFW 写入按节点串行；已有无 Panel 标记的等价人工 allow 可满足检查但不能在应用生命周期清理时删除。`openFirewall=true` 遇到 UFW 未安装时部署失败，不自动安装或启用；UFW 已安装但 inactive 时允许预写规则而不启用。应用 stop/purge 在容器删除后清理其受管规则，失败保持 Job 可重试并阻止 finalizer 提前完成。
...
- 反向代理设施应用依赖兼容 Agent 的 `RuntimeReconcile` 和 Docker 资源能力；未部署、不兼容或不可用的 Agent 不会处理设施应用配置，也不回退 SSH。服务器 traits 中的 `agent.reverse_proxy.enabled` 由设施应用部署服务器列表派生维护，仅用于 UFW 安装时自动放行反向代理端口，不提供独立节点开关。
...
- 启用 agent 后，读取类能力、软件包刷新/升级、UFW 状态及写操作、fail2ban 状态及配置应用、服务器重启、指标采集和应用运行时操作必须走 agent，不允许回退 SSH。应用运行时必须具备 `runtime-reconcile` capability；缺失时 Job 按 agent capability 不可用重试，但不因 capability 缺失单独触发重装。UFW 在 `agent.status=compatible` 且 `agent.url` 存在时按 Agent 准入，不再依赖旧记录中的 `privilege.mode` 或 `sudo_passwordless` 字段；无可用兼容 Agent 时仍要求 root 或免密 sudo。APT/UFW/fail2ban 由 Agent 参数化调用固定命令；服务器重启由 Agent 通过 `busctl` 调用 logind D-Bus `Reboot`。
- UFW 写操作必须保护服务器当前 SSH 端口：普通新增、普通删除与应用自动规则都在 Panel 和 Agent 两层拒绝该端口；只有安装/启用 UFW 前确保 SSH `ALLOW` 是内部例外。普通删除必须先解析当前编号对应的精确端口并在执行前重新读取校验，无法解析时拒绝；带 `panel:application:<applicationId>` 稳定 comment 的应用受管规则不能从普通 UFW 删除入口移除。Agent 内 UFW mutation 串行执行，避免规则编号及配置并发漂移。
...
- Panel 执行 agent 部署（`runDeployAgent`）时，只要 `agent.url` 已配置，就在任何停止/重启动作前先调用 `Health` 读取能力：能力包含 `prepare-restart` 且客户端实现 `RestartReadinessClient` 时调用 `PrepareRestart`（客户端与 Panel 侧均有最长 10 分钟的等待上限），收到 `ready` 才继续；能力缺失（旧 agent）、Health 或调用失败（含等待超时）时记录日志后直接继续；任务取消时中止部署。`GRPCClient` 同时实现可选的 `RestartReadinessProgressClient`，把流中的状态交给部署任务；协议要求 Agent 在等待期间每秒返回 `holdon`，因此首个状态 15 秒内未到达时视为流异常并降级继续部署。`holdon` 只表示 Agent 请求延迟重启，当前协议未携带原因，Panel 不得把它直接记录或展示为“软件包维护”；首次出现时立即记录原始状态，随后最多每 15 秒记录一次已等待时长，超时时明确记录等待上限和部署将继续。
...
- Stream reconnection uses exponential backoff (5s → 10s → … → 5min cap) that only grows across *consecutive* failures: if the previous connection delivered any report, the backoff resets to 5s before reconnecting, so a single transient blip never leaves the collector waiting minutes for the next attempt. Delivery is tracked per connection (a flag cleared on every new attempt), not by the persisted `last_message_at`; the silent-stream inspector only recycles connections that are currently established but idle, so an entry waiting out its backoff is never cancelled (cancelling it would reset the backoff and turn permanent outages into a 5s poll loop).
- Panel-side report handling is non-fatal by design: metrics save failures, container observation save failures, application reconcile trigger failures and misaligned sample timestamps are all logged (`Warn`/`Debug`) without breaking the stream; a container report whose save failed skips the reconcile trigger (the trigger runs only after a successful save, so the reconciler never acts on a failed snapshot) while the stream stays alive. Only stream-level transport errors end the stream.
...
- Agent 指标采集与上报解耦：CPU、内存、磁盘、网络、系统状态各由独立采样协程持续采集并写入内存缓存（一次采集后等待 1 秒再进行下一次）；上报循环是唯一提交方，到 Unix 对齐整点只从缓存读取快照并统一上报，因此单次采集耗时不会导致整点跳拍或延迟上报表。缓存尚未齐全（刚启动）或任一指标超过 15 秒未成功采样（持续失败）时上报不携带指标，Panel 保留既有数据，避免冻结的旧值伪装成新样本。
