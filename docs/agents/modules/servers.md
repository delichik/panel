...
- 服务器必须启用 agent，通过 traits 记录：`agent.enabled=true` 且 `agent.url=https://host:9786`。Panel 启动后会扫描服务器，调度器也会周期检查已配置 agent；没有配置 agent URL 的服务器会自动创建 `server_agent_deploy` 任务；已配置 agent 的服务器会执行健康检查，检查结果写入 `agent.status`、`agent.last_checked_at`、`agent.version` 和 `agent.last_error` traits。`agent.version` 跟随构建版本，仅用于展示和排查；兼容性由健康检查返回的 `capabilities` 和构建时自动生成的 agent HTTP contract hash 严格比较判断，普通 Panel 版本变化不得触发 agent 重部署。连续系统自动部署失败达到上限后进入 `undeployable`。
