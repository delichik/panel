...
- 服务器详情的 Agent 卡片不依赖隐藏任务中心作为唯一诊断入口：页面按 `serverId` 与 `server_agent_deploy` 查询最近任务，展示任务状态、部署阶段、错误和最近 20 条日志；运行中的任务每 2 秒增量读取日志，标签页不可见时暂停，刷新页面或切回服务器时恢复最近任务。重启就绪、Agent 返回的不透明 `holdon` 状态和等待超时日志在卡片内翻译为当前界面语言；界面不得在协议未提供原因时自行将 `holdon` 解释为软件包升级，其他远端自由文本保持原样。
