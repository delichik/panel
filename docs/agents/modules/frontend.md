...
- 应用页面操作语义跟随轻量控制平面：主操作“同步”表示保存/启用 desired state 后触发协调，不承诺当前请求内完成部署，也不强制重建已经满足 desired state 的成功节点；“停用”只写 disabled desired state 并触发协调停止现有实例；删除会提交清理期望并由协调任务清理运行时资源。设施应用的手动操作显示为“立即同步”，不使用“协调”作为用户按钮文案；入口网关页展示应用路由时必须提供跳转普通应用页的入口，因为这些路由在各应用的反向代理配置中维护。
- Runtime 应用实现：
