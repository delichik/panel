...
- 自签证书页面展示用户域 CA/TLS 证书，并单独展示带“系统内置”标签的 Panel 侧 Agent CA 与 Panel Agent 客户端证书。系统证书只能重置，不能删除、导入、导出或参与批量选择；每台服务器的 Agent 服务端证书不在该页逐台展示，控制用户域 CA/TLS 的 tab 必须放在系统内置证书区域下方，不能暗示会切换系统内置证书。
