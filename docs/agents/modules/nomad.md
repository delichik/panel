...
- SSH 管理地址与 Nomad `advertise` 地址是两个独立网络平面，Nomad `bind_addr` 必须保持 `0.0.0.0`。引导、加入、重部署、重建或切换 server/client 时，前端必须让用户显式选择 `advertiseAddress`；候选地址包括已保存 advertise、目标服务器探测到的物理网卡 IP，以及 SSH `host` 是有效 IP 时的 SSH host IP，用于阿里云、Oracle 等服务器只能探测到内网网卡但需要用公网 IP advertise 的场景。
- 后端将用户选择保存为服务器 trait `nomad.advertise_address`，并继续写入/读取 legacy `nomad.server_advertise_address` 以兼容旧数据；该地址用于 server/client 的 `advertise.http/rpc/serf`、Panel 的 Nomad HTTP 地址和 client 的 `server_join.retry_join`。
- 已存在 Nomad 任务但当前 server 没有 `nomad.advertise_address` 或 legacy `nomad.server_advertise_address` 时，控制平面返回 `migration_required`，前端进入类似首次初始化的重建引导，不继续展示可误操作的旧控制平面。
...
- 生成的 server/client 配置必须显式写入运行时 `region` 和 `datacenter`，并使用用户选择的 advertise 地址写入 `advertise.http/rpc/serf`；client 通过 `server_join.retry_join` 持续重试控制平面 RPC 地址，避免 server 短暂不可达时只留下本地存活但未注册的 agent。
...
- 前端 Nomad 节点页提交加入、重部署、重建或切换前必须让用户选择 Nomad advertise 地址；提交加入、重部署、重建、切换或移除后必须保留 `taskId`，并给出跳转任务中心的入口。
