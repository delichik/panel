...
- Normal and maintenance HTTPS listeners serve a process-global cached pair from `dataRoot/tls/panel.crt` and `dataRoot/tls/panel.key`. Runtime settings and key assets are only the synchronization source: changing the selected Panel certificate or reissuing/importing that asset serializes fixed-file activation with persistence, restores an in-memory fixed-pair snapshot if persistence fails, clears the cache on success, and lets the next new TLS connection lazily reload it. If the pair is absent or incomplete, the listener generates the managed self-signed certificate.
