...
> [!WARNING]
> 本文是非规范的历史实现说明，已由 [备份、还原与诊断验收规范](../acceptance/backup-restore-diagnostics.md) 替代。后续改动以稳定验收编号为准。
...
- Normal and maintenance HTTPS listeners serve a process-global cached pair from `dataRoot/tls/panel.crt` and `dataRoot/tls/panel.key`. Runtime settings and key assets are the synchronization source: normal startup first reconciles the database-backed RSA `panel-ca`/`panel-tls` chain, while a selected custom certificate remains authoritative when configured. Changing the selected Panel certificate or reissuing/importing that user asset serializes fixed-file activation with persistence, restores an in-memory fixed-pair snapshot if persistence fails, clears the cache on success, and lets the next new TLS connection lazily reload it. The fixed files are only a cache; if they are absent or invalid, the listener fails closed instead of generating an unregistered certificate. Maintenance mode does not assemble keyassets, so it may use an existing valid RSA or ECDSA fixed pair but reports an explicit error for missing, invalid, or legacy Ed25519 files.
