...
- 启动时若发现 pending export，Panel 不装配正常业务模块，而是进入备份导出维护模式。`ExportApp` 会短暂打开 app/task/metrics SQLite 执行 WAL checkpoint，立即关闭连接，再归档稳定文件。
...
启动期备份导出模式注册最小 API，且不要求登录认证，因为正常认证服务不会启动：
