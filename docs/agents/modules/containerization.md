...
- `apps where` 的应用主目录固定为 `/opt/panel/apps/<appID>`；同一节点同一应用只有一个实例，managed workspace 的 `files/archives/manifest/state` 和 `persistent` 均直接位于该主目录下，不再增加 `instances/<instanceID>` 层。Agent apply 会迁移旧实例目录，并重建仍引用旧 bind 路径的容器。
...
- Facility reverse proxy keeps the hidden application identity `facility-reverse-proxy` only for compatibility filtering. Its configuration remains in `facility_app_configs`; each save/reconcile now writes AppDB Instance desired state and a per-node Job, and the controller owns RuntimeReconcile. `/api/v1/activity` reads immutable execution and coordination events; facility current deployment state comes from Job/Instance observed data. 设施配置响应继续暴露 `reconcileStopped`；设施应用的 generation/spec_hash 只由设施模块维护，应用侧 `refreshApplicationSnapshot` 不得改写。
...
- Application reconciliation collectors read cached observations only and never replay that cache into instance observations. A server that reports a failed container, stale generation/spec hash, or managed file manifest drift can cause the application planner to create or reuse an AppDB Job for that server without redeploying other servers that are already healthy. Advisory report observations that lose CAS to an authoritative reconcile result are silently discarded instead of generating one warning per instance; durable Job lease/fencing rejection remains a structured warning.
...
- Agent report 的 `container_change` 触发在 collector 已确认应用/服务器发生停止、缺失、generation/spec hash 或 managed file drift 后，可绕过应用级 `application_reconcile_states.reconcile_next_run_at` 退避立即请求 planner；该绕过只作用于已 drift 的 app/server，不等同于 `force=true`，不得重部署同节点其它已满足 desired state 的应用。普通 scheduler 与非人工显式协调仍尊重应用级退避；HTTP deploy 等 `Manual=true` 人工协调必须绕过退避，但仍执行满足态过滤。 Agent 容器上报的 managed-file drift 判定基于本地 stat 指纹缓存（`state/managed-files.fingerprint.json`），未变化的文件不会反复哈希；事件触发的 container_change 快照与周期快照使用同一 drift 判定。
...
Agent 可靠步骤时间、输出及补传约定见 [agent-execution-events.md](agent-execution-events.md)，统一历史查询见 [activity.md](activity.md)。
