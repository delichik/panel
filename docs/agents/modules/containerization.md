...
- **协调追踪（reconcile trace）**：协调/部署全链路支持可开关的结构化追踪日志，默认关闭，通过运行时设置 `reconcile.trace`（设置页「协调追踪」开关）控制，settings 模块在启动加载与设置更新时同步。开启后输出统一携带 `trace="reconcile"` 与 `event` 字段的日志，事件覆盖：漂移判定（`drift`，含 missing/stopped/generation/spec_hash/managed_files 原因与期望 generation/hash）、健康（`healthy`）、退避跳过（`backoff`）、plan 请求与结果（`plan_requested`/`plan_result`/`plan_failed`/`plan_decision`，含 create/reuse/supersede/blocked 决策与 target 状态）、dispatcher 认领（`target_claimed`）、部署各步骤（`deploy_step_start`/`deploy_step_ok`/`deploy_step_failed`）、目标失败/成功与 lease 丢失（`target_failed`/`target_lease_lost`/`apply_succeeded`/`target_succeeded`）。追踪事件按 `app_id`/`server_id`/`target_id`/`task_id` 串联一次协调，用于定位协调器反复执行、部署反复失败等问题。Agent 侧容器操作（create/delete）另有常规 Info 日志，create 同名冲突自愈时记录占用者详情（`container name conflict; removing occupant before retry`，含占用者 ID/名称/状态/镜像/面板标签），不依赖该开关。
