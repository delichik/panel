...
- 静态资产 API 挂在 `/api/v1/facility-apps/reverse-proxy/static-assets`，只提供兼容的单资产上传、删除和认证下载，不提供通用资产 list；正式配置的 `staticAssets` 集合由入口代理自己的配置响应返回。新配置页不即时修改正式资产，而是随设施编辑会话提交。配置页与应用编辑器使用连续纵向配置流和统一文件行操作；静态文件的引用名称用于路由选择，下载文件名用于下载结果；新建文本资产未单独填写下载文件名时默认使用引用名称，替换传回原 `assetName`，删除被引用资产继续由 validate/commit 阻断，冲突提供明确的放弃修改并重新加载入口。
